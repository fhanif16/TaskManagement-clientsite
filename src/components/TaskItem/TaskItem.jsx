...
const TaskItem = () => {
      ...
      if (user?.email) {
        ...
        try {
          const response = await fetch(
            `https://task-server-side-eta.vercel.app/task?email=${user.email}`
          );
          if (!response.ok) {
  ...
  const handleCategoryChange = async (id, newCategory) => {
    ...
    try {
      const response = await fetch(
        `https://task-server-side-eta.vercel.app/task/${id}/category`,
        {
          method: "PATCH",
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const response = await fetch(
        `https://task-server-side-eta.vercel.app/task/${id}`,
        {
          method: "DELETE",
        }
      );
